...
这是 [xcxcadc/chenfei-Glass-Prism-dns](https://github.com/xcxcadc/chenfei-Glass-Prism-dns) 的中文增强版本。默认提供简体中文界面、自定义服务域名与分类、服务级解锁机切换、IP 配置闭环、解锁链路流量统计、账户安全和客户端脚本。当前客户端工具版本为 `1.4.9`，加密传输版本为 `2.2.2`；完整说明见 [ENHANCED_ZH.md](ENHANCED_ZH.md)，增强层版本见 [Releases](https://github.com/xcxcadc/chenfei-Glass-Prism-dns/releases/latest)。
...
- **自定义服务** - 可在前端新增、编辑、删除任意服务名称与域名；支持 `example.com` 和 `*.example.com` 泛域名，保存后会保留原格式并安全编译为去重的 `DOMAIN-SUFFIX`
...
- **真实兼容性优先** - 每项已选服务会核对全部路由域名、抑制 AAAA，并检查浏览器实际依赖；第三方检测被 WAF 拒绝时，只有 DNS 与浏览器依赖全部通过才显示链路可用
...
- **受限网络传输** - 目标机可通过加密 TCP SNI 传输连接解锁机；每 30 秒用真实 HTTPS 路径检查隧道，发现“TCP 假在线”时只重建该隧道，不改变用户选择的 SG、VN 等服务线路
- **全域名实测** - 每项服务逐一核对全部路由域名和浏览器依赖，不再只抽查一个首页域名；结果会给出 `DNS x/x` 与 `HTTPS y/y`
- **共存守护** - 仅监控并恢复 `prism-agent`，不会重启或覆盖同机 MTProxy、XrayR、V2bX
- **服务品牌图标** - 服务卡片和 IP 选择器立即显示本地占位并异步替换为对应品牌图标；面板启动时优先预热已配置服务并把真实图标持久化到数据目录，重启后无需重新等待
...
“节点管理”显示解锁机自身的 Agent UnlockTests；“IP 配置”会在目标服务器上运行与用户提供的 `dns_unlock.sh` 相同的 UnlockTests，并把原始结论回传到每个已选服务。服务配置页优先显示目标机实测结果，同时核对全部路由域名、AAAA 抑制及服务所需的浏览器依赖。目标机审计在路由变化后自动执行并定期刷新，但只报告结果，不会修改用户选择的解锁机。
...
页面生成的专属命令只用于目标机首次安装；后续在面板保存新增、取消或切换服务会自动生效，不再弹出安装命令。通用工具支持安装/连接 DNS Client Agent、本机 DNS 测试、永久或临时设置系统 DNS、自动备份、恢复及状态检查。安装器会备份并停用冲突的旧 `dnsmasq`/`sniproxy`，但不会修改 MTProxy、XrayR 或 V2bX。每台新目标机默认安装通用配置哈希守卫，每 10 秒读取该机器自己的面板地址和令牌；路由变化时仅安全重启 `prism-agent`，等待 53 端口和全部 IPv4 路由恢复后再提交新哈希，避免上游热更新残留旧 DNS 缓存。受限网络中的目标机会使用加密 TCP SNI 传输代替不稳定的跨境 UDP 53；服务审计逐一检查全部路由域名与浏览器依赖，长任务运行期间不会被 10 秒守卫重复排队。Proxy 每 5 秒刷新面板授权白名单，未纳管 IPv4 无法使用 DNS 53 或 SNI 80/443。首次安装要求全部所选域名正确映射到配置的解锁机；第三方服务的 HTTPS 探测失败只会告警，不会因地区策略、限流或瞬时握手异常阻断系统 DNS 接管。安装完成后还会创建专用 nftables 计数器；systemd timer 在启用后 15 秒内首次上报，之后每分钟按目标 IP 统计本机 Prism DNS 的 UDP/TCP 53 与所选解锁机 TCP 80/443 完整 RX/TX。UnlockTests 审计前后都会清除计数器，检测流量不会计入用户用量。

以上守卫、IPv4 优先、AAAA 抑制、只读实测、授权白名单、流量统计和自动应用逻辑均从面板数据动态生成，不包含固定 IP，可直接用于后续新增的任意解锁机和被解锁机。实测结果不会触发节点跳转；如需更换线路，必须在面板手动选择并保存。
