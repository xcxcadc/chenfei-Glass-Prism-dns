...
5. 左上角网页名称和浏览器标签标题正确。
6. 随机选择一个目标 IP，执行 Gemini、Claude、Disney+ 或 YouTube 的真实解锁测试，确认同时显示 A 映射、AAAA 抑制、TLS/SNI 握手和代表页面/服务方结论。
...
增强层首次启动会自动迁移旧数据：移除持久化 Proxy IPv6，只保留受管解锁所需的 IPv4；共用父域、子域或泛域名的服务会归一到同一 Proxy；`1.4.10` 会删除旧版自动故障切换记录，但保留用户当前选择的服务路由。迁移会保留节点 ID、目标 IP、令牌、累计流量和最后采样基线，并请求目标机重新执行 A、AAAA、TLS/SNI 与代表页面/服务方四阶段实测。目标机重新执行专属命令后会默认安装 `127.0.0.1:5353` 专用 dnsmasq、动态路由文件和 nftables 本地 53 重定向；这些运行时文件由面板配置重新生成，不需要从旧目标机复制。加密传输 `2.2.3` 把客户端全量同步调整为 60 秒，同时保留 SSH 15 秒保活与三次失联判定。新版实测只报告结果，不会再次自动切换节点；以后新增的解锁机和目标机默认继承相同逻辑。
...
重新执行安装命令只更新 Prism Agent、面板地址和守卫配置，不会修改 MTProxy、XrayR 或 V2bX。稳定安装器会验证并锁定上游 Agent `v1.2.1`，避免迁移后被 `v1.3` 被动熔断回退公网 DNS；安装和卸载过程会自动解除旧文件锁。Proxy 的 IPv4 DNS/SNI 授权白名单会在新面板恢复后 5 秒刷新；IPv6 仅拒绝 Prism 的 53/80/443，MTProxy 等其他端口不受影响。
