...
增强层首次启动会自动迁移旧数据：移除持久化 Proxy IPv6，只保留受管解锁所需的 IPv4；共用父域、子域或泛域名的服务会归一到同一 Proxy；`1.4.5` 会删除旧版自动故障切换记录，但保留用户当前选择的服务路由。迁移会保留节点 ID、目标 IP、令牌、累计流量和最后采样基线，并请求目标机重新执行 A、AAAA、TLS/SNI 与代表页面/服务方四阶段实测。目标机重新执行专属命令后会默认安装 `127.0.0.1:5353` 专用 dnsmasq、动态路由文件和 nftables 本地 53 重定向；这些运行时文件由面板配置重新生成，不需要从旧目标机复制。加密传输 `2.2.3` 把客户端全量同步调整为 60 秒，同时保留 SSH 15 秒保活与三次失联判定。新版实测只报告结果，不会再次自动切换节点；以后新增的解锁机和目标机默认继承相同逻辑。
