...
- 已安装目标机修改服务后自动生效：每 10 秒配置哈希守卫在路由变化时安全重启 Agent，清除上游热更新遗留的旧 DNS 缓存。
- 受限网络目标机通过加密 TCP SNI 传输连接解锁机；每 30 秒执行真实 HTTPS 路径探测，发现 SSH 端口仍开但业务链路卡死时只重建对应隧道，不改变逐服务 SG/VN 选择。
- 解锁审计逐一检查全部路由域名、AAAA 抑制及浏览器依赖，并显示 `DNS x/x` 与 `HTTPS y/y`；不再只抽查一个首页域名。
- 守护任务只操作 `prism-agent`，不会重启或覆盖同机 MTProxy、XrayR、V2bX。
- 选择服务时原地更新勾选状态并保持滚动位置；图标未返回前立即显示本地首字占位，增强层启动时优先预热已配置服务，并把真实品牌图标持久化到 `/var/lib/prism-enhancer/icon-cache`，重启后直接复用。
...
`prismdns.sh` 会为每台目标服务器创建独立 nftables 统计链：本机 Prism Agent 的 UDP/TCP 53 请求与响应计入 DNS 用量，只有与当前所选 Proxy IPv4 之间的 TCP 80/443 才计入 SNI 解锁用量。普通公网 80/443、系统更新和其他非解锁机流量不会计入。systemd timer 启用后 15 秒内首次上报，之后每分钟上报一次；首次上报或清零后的首次上报只建立新基线。自动 UnlockTests 审计前后都会重置本地计数器，避免把检测流量计入用户用量。
...
`prismdns.sh 1.4.9` 会在每台目标机安装 `/usr/local/lib/prismdns/sync-routes.sh` 和 `prismdns-route-sync.timer`。增强层会把服务到 Proxy IPv4 的映射持久化，并在每次 Agent `/api/sync` 时重建 A 规则、抑制对应 AAAA，因此不再依赖上游 Controller 的内存 override。受限网络链路会通过 `prism_transport.sh 2.2.2` 建立加密 TCP SNI 传输，并用双站点 HTTPS 探针识别“SSH/TCP 仍在线但业务转发卡死”的假健康；修复时只重建对应隧道，不改服务路由。守卫每 10 秒使用该机器自己的面板地址和令牌获取配置，同时实际解析全部健康探针；配置变化、Agent 假活、53 端口失效或路由不再指向所选 IPv4 解锁机时，会以 60 秒限频安全重启 `prism-agent`，确认全部探针恢复后才提交新哈希。守卫同时限制 Agent 文件描述符和异常连接泄漏，并且只操作 Prism 服务，不触碰 MTProxy、XrayR、V2bX。首次接入要求全部所选域名正确映射到配置的解锁机；第三方服务 HTTPS 探测仅用于报告真实可用性，不会因地区限制、限流或瞬时 TLS 握手失败阻断系统 DNS 接管。面板重启或网络瞬时不可达时，上报与守卫任务会正常退出并等待下一轮，不产生 systemd 失败误报。

目标机的 UnlockTests 只检测该 IP 已选择的服务，并连续运行三次。每项服务还会逐一验证全部路由域名是否准确解析到所选 Proxy IPv4、对应 AAAA 是否被抑制，以及 Gemini、Claude、ChatGPT 等应用所需的浏览器依赖是否全部可达。`UNSTABLE`、`Banned` 或 WAF 只代表第三方脚本探针受限；只有完整 DNS 与浏览器路径同时通过时才显示“可用（第三方探针受限）”。稳定返回 `YES` 的第三方项目会保留 `YES` 并附加 DNS/HTTPS 明细，不再因为其没有可计数的首页跳转而误报降级。任何实测结果都不会修改路由或自动跳转节点，用户需要切换线路时必须手动选择并保存。长时间审计运行中会被守卫识别并去重，不会每 10 秒重复排队。节点页会汇总每台目标 IP 的实测结果，解锁机 Agent 自检仅保留为参考，避免把解锁机本机的旧检测结果当成客户端实际可用性。
...
`1.4.9` 回归使用运行时最新有效服务目录，并对每台目标机逐项验证全部 DNS 映射和浏览器依赖。面板不会把解锁机自身自检结果伪装成目标机可用，不会因第三方检测波动自动切换节点，也不会把共享域名拆到互相覆盖的出口。Crackle、Salto、GYAO 已停止运营，不再进入有效服务目录。

## 解锁检测与账户安全

“节点管理”中的“运行解锁检测”调用解锁机 Agent 的检测任务，完成后直接汇总可用与不可用项目，但这只代表解锁机自身。目标 IP 安装 `prismdns.sh` 后，会在路由变化及定期周期内运行与 `dns_unlock.sh` 相同的 `/usr/bin/ut -m 4 -f 20 -b=false -s=false`，再逐一核对全部路由域名、AAAA 抑制和已知浏览器依赖，把结果上报到“IP 配置”。服务配置页优先展示目标机结论；审计只写报告，不修改路由。
