...
`prismdns.sh 1.5.18` 会在每台目标机安装 `/usr/local/lib/prismdns/sync-routes.sh`、`prismdns-route-sync.timer`、Prism 专用 dnsmasq 和 DNS 守卫。增强层把服务到用户所选 Proxy IPv4 的映射持久化并生成 `127.0.0.1:5353` 的确定性路由；路由守卫每 10 秒刷新配置哈希，并为活动的 XrayR、V2bX、sing-box、Hysteria、TUIC、Trojan 等代理进程建立独立 nftables cgroup 规则，把代理内置的外部 DNS 请求转交到该端口，已选服务只返回 IPv4 路由并抑制 AAAA，不覆盖代理、MTProxy 或 Docker 配置。Agent 保留用于面板同步、授权和报告。配置变化、30 分钟健康缓存刷新和 `check.unlock.media` IPv4 服务审计仍会全量验证所有域名；健康上报会把 DNS 守卫或专用 DNS 缺失明确标为异常。安装期只强制检查 DNS 路由是否已经映射到面板所选解锁机，完整 HTTPS/流媒体可用性由后台审计继续跑，避免第三方站点瞬时超时拖死新机上线。受限网络链路会通过 `prism_transport.sh 2.3.1` 建立加密 TCP SNI 传输；Proxy 同步时会补装缺失的 egress 转发器，客户端每 60 秒同步，Proxy 每 5 秒在每次同步完成后再次注册，并用双站点 HTTPS 探针识别“SSH/TCP 仍在线但业务转发卡死”的假健康，SSH 本身继续按 15 秒保活、连续 3 次失联判定；修复时只重建对应隧道，不改服务路由。发往已选解锁机的 UDP/443 会被明确拒绝，促使支持 QUIC 的应用回落到 SNIproxy 可处理的 TCP/TLS。每分钟定时器只负责流量上报，耗时的四阶段检测由独立 `prismdns-service-audit.service` 执行。面板或网络瞬时不可达时任务会正常退出并等待下一轮。
